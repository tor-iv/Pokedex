...
class _pokemon_database:
    ...
    def get_pokemon(self, pID):
        ...
        try:
            for i in self.pokemon_data:
                if lower(pID) = i['name']['english']:
                    name = self.pokemon_data[i]['name']['english']
                    type = self.pokemon_data[i]['type']
                    pokemon = list((name, type))
    ...
    def delete_pokemon(self, name):
        for i in self.pokemon_data:
            if i['names']['english'] == name:
                del(self.pokemon_names[i])
                del(self.pokemon_genres[i])
